...
describe('test BasePool', function () {
    ...
    describe('contract execution', function () {
        ...
        describe('liquidity', function () {
            it.only('deposits liquidity', async function () {
                ...
                await token1Contract.connect(deployer).approve(positionManagerContract.address, '1000')
                hre.tracer.enabled = true
                const tx = await positionManagerContract.connect(deployer).mint(mintParams)
                console.log(await tx.wait())
                hre.tracer.enabled = false

            })
            it('withdraws liquidity', async function () {
                // TODO
            })
